...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
    ...
    else if (strcmppgm2ram((char*)name, (ROM char*)"ui/getarch.cgi") == 0) {
        ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"mem");
        if (!ptr) { return HTTP_IO_DONE; }
        else if ( strcmppgm2ram((char*)ptr, (ROM char*)"spi") == 0 ) {
            
        }
        else if ( strcmppgm2ram((char*)ptr, (ROM char*)"usb") == 0 ) {
            
        }
        
        ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"fname");
...
void HTTPPrint_showarch (void) {
    ...
    res = f_opendir(&dir, "0:/");
    if (res == FR_OK) {
        if (!first_disk) TCPPutROMString(sktHTTP, (ROM void*)",\n");
        TCPPutROMString(sktHTTP, (ROM void*)"\"spi\" : [\n");
        for (i=0;;i++) {
            res = f_readdir(&dir, &fno);
            if (res != FR_OK || fno.fname[0] == 0) break;
            if (!(fno.fattrib & AM_DIR)) { // It is not a directory
                TCPPutROMString(sktHTTP, (ROM void*)((i > 0) ? ",\n\t{" : "\t{"));
                //json data constructed here
                TCPPutROMString(sktHTTP, (ROM void*)"\"filename\" : \"");
                TCPPutString(sktHTTP, fno.fname);
                TCPPutROMString(sktHTTP, (ROM void*)"\", \"filesize\" : ");
                snprintf(buff, sizeof(buff)-2, "%lu}\n", fno.fsize);
                TCPPutString(sktHTTP, buff);
            }    
        }
        TCPPutROMString(sktHTTP, (ROM void*)"]");
    ...
    res = f_opendir(&dir, "1:/");
        if (res == FR_OK) {
        if (!first_disk) TCPPutROMString(sktHTTP, (ROM void*)",\n"); 
        TCPPutROMString(sktHTTP, (ROM void*)"\"usb\" : [\n");
        for (i=0;;i++) {
            res = f_readdir(&dir, &fno);
            if (res != FR_OK || fno.fname[0] == 0) break;
            if (!(fno.fattrib & AM_DIR)) { // It is not a directory
