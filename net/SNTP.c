 ...
 *					Micrrtochip C30 v3.12 or higher
...
#else
#define NTP_SERVER      AppConfig.NtpServer
//#define NTP_SERVER      "ntp1.tp.pl"
...
// case SM_UDP_IS_OPENED:
		...
		case SM_UDP_RECV:
            ...
            #ifdef SNTP_UPDATE_RTCC
                struct tm *current_time = gmtime(&dwSNTPSeconds);
                rtccTime time;
                rtccDate date;
                
                time.l = 0;
                date.l = 0;
                time.hour = ByteToBcd(current_time->tm_hour);
                time.min = ByteToBcd(current_time->tm_min);
                time.sec = ByteToBcd(current_time->tm_sec);
                date.mday = ByteToBcd(current_time->tm_mday);
                date.mon = ByteToBcd(current_time->tm_mon);
                date.year = ByteToBcd(current_time->tm_year);
